...
import { currentSlot } from '../lib/startlist'
...
function currentLabel(rt: ParcoursRuntime): string {
  ...
  return s ? `${s.startNr} · ${s.vorname} ${s.nachname} (Lauf ${slot.lauf})` : '–'
}
...
<template>
  <div class="page">
    ...
    <div class="card row">
      ...
      <span class="dim small">Wartezeiten sind Schätzungen aus den gemessenen Startabständen.</span>
    </div>
    ...
    <section v-for="parcours in store.state.parcoursList" :key="parcours.id" class="card">
      ...
      <div class="scroll-x">
        <table>
          ...
          <tbody>
            <tr v-for="row in rowsFor(runtimeOf(parcours.id)!)" :key="row.slotId">
              <td class="mono">{{ row.startNr }}</td>
              <td>{{ row.name }}</td>
              <td class="dim">{{ row.verein }}</td>
              <td>
                <span class="badge" :style="{ background: classColor(row.klasse) }">
                  {{ row.klasse }}
                </span>
              </td>
              <td>{{ row.lauf }}</td>
              <td>{{ formatWait(row.wait) }}</td>
            </tr>
